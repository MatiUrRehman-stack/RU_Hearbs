...
function OurStory() {
  return (
    <section
      id="our-story"
      className="relative overflow-hidden px-4 sm:px-6 py-16 sm:py-20 lg:py-28"
    >
      {/* Soft decorative blobs, consistent with Hero */}
      <div className="pointer-events-none absolute top-0 -right-24 h-72 w-72 rounded-full bg-(--sage-bg) blur-3xl opacity-50" />
      <div className="pointer-events-none absolute bottom-0 -left-20 h-72 w-72 rounded-full bg-(--accent-bg) blur-3xl opacity-40" />

      <div className="relative z-10 max-w-6xl mx-auto">
        {/* Section intro */}
        <div className="flex flex-col items-center gap-4 text-center max-w-2xl mx-auto mb-14 sm:mb-20">
          ...
          <p className="text-[15px] sm:text-[17px] leading-[165%] text-(--text)">
        ...
        {/* Photo + narrative */}
        <div className="grid grid-cols-1 lg:grid-cols-2 items-center gap-12 lg:gap-16 mb-16 sm:mb-24">
          {/* Photo composition */}
          <div className="relative mx-auto w-full max-w-70 sm:max-w-sm pb-8 pl-8 sm:pb-10 sm:pl-10 lg:pb-12 lg:pl-12">
            <div
              className="group relative aspect-4/5 w-full overflow-hidden rounded-[20px] sm:rounded-[28px] shadow-(--shadow)
            ...
            <div
              className="group absolute -bottom-2 -right-2 sm:-bottom-4 sm:-right-4 h-[45%] w-[52%] overflow-hidden
                rounded-xl sm:rounded-2xl border-4 border-(--bg) shadow-(--shadow)
          ...
          <div className="flex flex-col items-start gap-5 sm:gap-6 text-left">
            ...
            <p className="max-w-md text-[15px] sm:text-[16px] lg:text-[17px] leading-[165%] text-(--text)">
        ...
        {/* Milestone timeline */}
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-8 sm:gap-6 mb-16 sm:mb-20">
          {MILESTONES.map(({ year, title, text }) => (
            <div
              key={year}
              className="flex flex-col items-start gap-2 text-left rounded-2xl border border-(--border)
                bg-(--bg-soft) p-6 transition-all duration-300 ease-out
                hover:-translate-y-1 hover:shadow-(--shadow)"
            >
              <span className="text-[13px] font-semibold uppercase tracking-[1.5px] text-(--accent)">
                {year}
              </span>
              <h4 className="m-0! text-[18px] font-semibold text-(--text-h)">
                {title}
              </h4>
        ...
        <div className="flex flex-col lg:flex-row items-center gap-8 lg:gap-12 rounded-[28px] border border-(--border) bg-(--bg-soft) p-6 sm:p-10">
          <div className="w-full max-w-70 lg:max-w-xs shrink-0 overflow-hidden rounded-2xl shadow-(--shadow) rotate-2 transition-transform duration-500 ease-out hover:rotate-0">
            <img
          ...
          <div className="flex flex-col items-start gap-4 text-left">
            ...
            <p className="max-w-lg text-[15px] sm:text-[16px] leading-[165%] text-(--text)">
